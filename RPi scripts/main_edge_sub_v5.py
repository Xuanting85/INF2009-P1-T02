...
def on_message(client, userdata, message):
    ...
    detectAccSpikespikes(acc_data[0], acc_data[1], acc_data[2])
    detectGyroSpikespikes(gyro_data[0], gyro_data[1], gyro_data[2])
...
fig, (ax_acc, ax_gyro) = plt.subplots(2, 1, figsize=(8, 6))
...
# Function to update the accelerometer plot
def update_plot_acc(frame):
    ...
    ax_acc.relim()  # Update the limits of the axes
    ax_acc.autoscale_view()  # Autoscale the view
    return line_acc_x, line_acc_y, line_acc_z

# Function to update the gyroscope plot
def update_plot_gyro(frame):
    ...
    ax_gyro.relim()  # Update the limits of the axes
    ax_gyro.autoscale_view()  # Autoscale the view
